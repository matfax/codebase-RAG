...
async def test_self_indexing():
    ...
    try:
        ...
        index_result = await app.call_tool("index_directory", {"directory": project_root, "clear_existing": True})
        print(f"Indexing result: {index_result}")

        # Search for specific functionality
        print("\nSearching for 'MCP server'...")
        search_result = await app.call_tool("search", {"query": "MCP server", "n_results": 5})
        print(f"Search result: {search_result}")

        # Search for FastMCP usage
        print("\nSearching for 'FastMCP'...")
        fastmcp_search = await app.call_tool("search", {"query": "FastMCP", "n_results": 3})
