...
class TestCachePerformanceMonitoring(TestPerformanceMonitoringBase):
    ...
    @pytest.mark.asyncio
    async def test_operation_timing_monitoring(self, cache_service, performance_service):
    ...
    @pytest.mark.asyncio
    async def test_throughput_monitoring(self, cache_service, performance_service):
    ...
    @pytest.mark.asyncio
    async def test_hit_rate_monitoring(self, cache_service, performance_service):
    ...
    @pytest.mark.asyncio
    async def test_error_rate_monitoring(self, cache_service, performance_service):
...
class TestMemoryPressureMonitoring(TestPerformanceMonitoringBase):
    ...
    @pytest.mark.asyncio
    async def test_memory_usage_tracking(self, cache_service, memory_service):
        ...
        await memory_service.set_thresholds({"warning": 0.7, "critical": 0.9, "emergency": 0.95})  # 70%  # 90%  # 95%
    ...
    @pytest.mark.asyncio
    async def test_automatic_memory_management(self, cache_service, memory_service):
        ...
        for i in range(100):
            await cache_service.set(f"memory:test:{i}", {"data": "x" * 1000, "priority": i % 3, "created": time.time()})
    ...
    @pytest.mark.asyncio
    async def test_memory_leak_detection(self, cache_service, memory_service):
...
class TestMetricsCollectionAndReporting(TestPerformanceMonitoringBase):
    ...
    @pytest.mark.asyncio
    async def test_comprehensive_metrics_collection(self, cache_service, performance_service, memory_service, metrics_collector):
    ...
    @pytest.mark.asyncio
    async def test_historical_metrics_tracking(self, cache_service, performance_service, metrics_collector):
        """Test historical metrics tracking and analysis."""
        cache_service.enable_performance_monitoring(performance_service)
        await metrics_collector.register_source("performance", performance_service)

        # Configure historical tracking
        await metrics_collector.enable_historical_tracking(interval_seconds=0.1, retention_hours=1)  # Fast for testing
        ...
        historical_data = await metrics_collector.get_historical_metrics(metric_name="operations_per_second", time_range_minutes=1)
    ...
    @pytest.mark.asyncio
    async def test_performance_alerting(self, cache_service, performance_service):
    ...
    @pytest.mark.asyncio
    async def test_performance_optimization_recommendations(self, cache_service, performance_service):
...
class TestPerformanceBenchmarking(TestPerformanceMonitoringBase):
    ...
    @pytest.mark.asyncio
    async def test_performance_baseline_establishment(self, cache_service, performance_service):
    ...
    @pytest.mark.asyncio
    async def test_performance_regression_detection(self, cache_service, performance_service):
        ...
        latency_regression = next((r for r in regression_analysis["regressions"] if r["metric"] == "latency"), None)
        assert latency_regression is not None
        assert latency_regression["regression_factor"] > 5  # 5x worse than baseline

    @pytest.mark.asyncio
    async def test_load_testing_integration(self, cache_service, performance_service):
        ...
        load_scenarios = [
            ...
            {"name": "burst_load", "operations_per_second": 500, "duration_seconds": 0.5, "operation_mix": {"get": 0.8, "set": 0.2}},
        ]
