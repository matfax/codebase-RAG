...
''')
        ...
        for query in construct_queries:
            ...
            for result in search_result['results']:
                ...
                if 'chunk_type' in result:
                    ...
                    assert chunk_type in ['function', 'class', 'method', 'interface', 'constant']
    ...
    def test_incremental_indexing_workflow(self, sample_project):
        ...
        # Initial indexing
        initial_result = index_directory(
            directory=sample_project,
            clear_existing=True,
            incremental=False
        )
        assert initial_result['success'] == True
        initial_points = initial_result['total_points']
...
''')
        ...
        incremental_result = index_directory(
            directory=sample_project,
            incremental=True
        )
        
        # Should detect and process the new file
        assert incremental_result['success'] == True
...
''')
        ...
        try:
            ...
            cross_search = search(
                query="distinctive_functionality",
                cross_project=True,
                n_results=5
            )
...
class TestRealWorldScenarios:
    ...
    def test_configuration_search(self, sample_project):
        ...
        for query in config_queries:
            ...
            found_config = False
            for result in search_result['results']:
                if 'config' in result.get('file_path', '').lower() or \
                   '.json' in result.get('file_path', ''):
                    found_config = True
                    break
    ...
    def test_code_discovery_workflow(self, sample_project):
        ...
        for query in discovery_queries:
            ...
            for result in search_result['results']:
                ...
                has_code_markers = any(marker in content for marker in [
