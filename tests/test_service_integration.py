...
class TestCacheServiceIntegration(TestServiceIntegrationBase):
    ...
    @pytest.mark.asyncio
    async def test_cache_with_invalidation_integration(
    ...
    @pytest.mark.asyncio
    async def test_cache_with_cascade_invalidation(
        self, cache_service, invalidation_service
    ):
    ...
    async def test_memory_pressure_integration(self, cache_service):
        ...
        for i in range(100):
            await cache_service.set(
                f"memory:test:{i}",
                {"data": "x" * 1000, "priority": i % 3}  # Varying priorities
            )
...
class TestFileMonitoringIntegration(TestServiceIntegrationBase):
    ...
    @pytest.mark.asyncio
    async def test_file_change_cache_invalidation(
        self, cache_service, invalidation_service, monitoring_service
    ):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            await monitoring_service.register_file_cache_mapping(
                str(file1), ["cache:model1:*"]
            )
            await monitoring_service.register_file_cache_mapping(
                str(file2), ["cache:model2:*"]
            )
    ...
    @pytest.mark.asyncio
    async def test_batch_file_changes(
        self, cache_service, invalidation_service, monitoring_service
    ):
...
class TestErrorPropagationIntegration(TestServiceIntegrationBase):
    ...
    async def test_cascade_error_handling(self, cache_service, invalidation_service):
        ...
        # Register dependencies
        await cascade_service.register_dependency(
            "parent:1", ["child:1", "invalid:key", "child:2"]
        )
        ...
        result = await cascade_service.invalidate_cascade(
            "parent:1", continue_on_error=True
        )
