...
from src.utils.invalidation_event_bus import InvalidationEventBus, InvalidationEvent
...
class TestFileChangeInvalidationWorkflow(TestInvalidationWorkflowBase):
    ...
    @pytest.mark.asyncio
    async def test_single_file_change_workflow(
        self, cache_service, invalidation_service, file_monitoring_service
    ):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            await file_monitoring_service.register_file_cache_mapping(
                str(user_model_file),
                cache_patterns=["model:user:*", "schema:user"]
            )
    ...
    @pytest.mark.asyncio
    async def test_batch_file_changes_workflow(
        self, cache_service, invalidation_service, file_monitoring_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_file_rename_workflow(
        self, cache_service, invalidation_service, file_monitoring_service
    ):
...
class TestCascadeInvalidationWorkflow(TestInvalidationWorkflowBase):
    ...
    @pytest.mark.asyncio
    async def test_deep_cascade_workflow(
        self, cache_service, invalidation_service, cascade_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_selective_cascade_workflow(
        self, cache_service, invalidation_service, cascade_service
    ):
        ...
        result = await cascade_service.invalidate_cascade(
            "project:123",
            filter_fn=important_cache_filter
        )
    ...
    @pytest.mark.asyncio
    async def test_cascade_with_circular_dependency_handling(
        self, cache_service, cascade_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_cascade_failure_recovery_workflow(
        self, cache_service, cascade_service
    ):
...
class TestInvalidationEventWorkflow(TestInvalidationWorkflowBase):
    ...
    @pytest.mark.asyncio
    async def test_event_driven_invalidation_workflow(
        self, cache_service, invalidation_service, event_bus
    ):
    ...
    @pytest.mark.asyncio
    async def test_event_aggregation_workflow(
        self, cache_service, invalidation_service, event_bus
    ):
    ...
    @pytest.mark.asyncio
    async def test_event_priority_workflow(
        self, cache_service, invalidation_service, event_bus
    ):
        ...
        # Publish events
        critical_event = InvalidationEvent(
            cache_key="critical:data",
            event_type="invalidated",
            priority=10,
            timestamp=time.time()
        )
        
        normal_event = InvalidationEvent(
            cache_key="normal:data",
            event_type="invalidated",
            priority=5,
            timestamp=time.time()
        )
...
class TestComplexInvalidationWorkflows(TestInvalidationWorkflowBase):
    ...
    @pytest.mark.asyncio
    async def test_microservice_invalidation_workflow(
        self, cache_service, invalidation_service, cascade_service, event_bus
    ):
        ...
        await cascade_service.register_dependency(
            "user:123",  # User service
            ["order:user:123", "notification:user:123", "analytics:user:123"]
        )
    ...
    @pytest.mark.asyncio
    async def test_tenant_isolation_invalidation_workflow(
        self, cache_service, invalidation_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_real_time_invalidation_workflow(
        self, cache_service, invalidation_service, file_monitoring_service, event_bus
    ):
