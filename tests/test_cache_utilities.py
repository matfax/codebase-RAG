...
from src.models.cache_models import CacheEntry, CacheMetrics, CachePolicy, CacheStatus, EvictionPolicy, InvalidationRule, SecurityContext
from src.utils.cache_key_utils import create_namespace_key, generate_cache_key, get_key_prefix, parse_cache_key, sanitize_key_component
...
class TestCacheModels:
    ...
    def test_cache_entry_expiration(self):
        ...
        entry = CacheEntry(key="expired:key", value="data", ttl=3600, created_at=past_time)  # 1 hour TTL
    ...
    def test_invalidation_rule_matching(self):
        ...
        rule = InvalidationRule(pattern="cache:user:*", dependencies=["cache:profile:*", "cache:preferences:*"], cascade=True, priority=10)
    ...
    def test_security_context_validation(self):
        ...
        context = SecurityContext(
            user_id="user123", tenant_id="tenant456", permissions=["read", "write"], encryption_required=True, audit_enabled=True
        )
...
class TestCacheWarmupUtils:
    ...
    async def test_scheduled_warmup(self):
        ...
        schedule = {
            "daily_reports": {"pattern": "report:daily:*", "schedule": "0 6 * * *", "priority": "high"},  # 6 AM daily
            "user_profiles": {"pattern": "user:profile:*", "schedule": "*/30 * * * *", "priority": "medium"},  # Every 30 minutes
        }
...
class TestSecureCacheUtils:
    ...
    def test_secure_cache_wrapper(self):
        ...
        security_context = SecurityContext(user_id="user123", encryption_required=True)
    ...
    def test_validate_security_context(self):
        ...
        # Valid context
        valid_context = SecurityContext(user_id="user123", tenant_id="tenant456", permissions=["read", "write"])
        ...
        invalid_context = SecurityContext(user_id=None, permissions=["read"])
...
class TestCacheUtilityIntegration:
    ...
    async def test_secure_cache_with_warmup(self):
        ...
        entries = [{"key": "secure:user:123", "data": {"name": "Test", "ssn": "123-45-6789"}}]
    ...
    def test_cache_key_with_security_namespace(self):
        ...
        security_context = SecurityContext(user_id="user123", tenant_id="tenant456")

        # Generate tenant-isolated key
        key = generate_cache_key("users", "profile", namespace=f"tenant:{security_context.tenant_id}")
    ...
    def test_invalidation_with_secure_keys(self):
        ...
        rule = InvalidationRule(pattern="tenant:*:users:*", cascade=True, security_context=SecurityContext(tenant_id="tenant456"))
