...
from redis.exceptions import ClusterDownError, ConnectionError, ReadOnlyError, RedisError, TimeoutError
...
from src.utils.resilient_redis_manager import RedisClusterManager, RedisConnectionPool, RedisHealthChecker, ResilientRedisManager
...
class TestResilientRedisManager:
    ...
    async def resilient_manager(self):
        ...
        config = RedisConfig(host="localhost", port=6379, db=15, retry_attempts=3, retry_delay=0.1, circuit_breaker_enabled=True)
    ...
    async def test_connection_pool_exhaustion(self, resilient_manager):
        ...
        try:
            extra_conn = await asyncio.wait_for(resilient_manager.get_connection(), timeout=1.0)
...
class TestRedisClusterSupport:
    ...
    async def cluster_manager(self):
        ...
        cluster_nodes = [{"host": "localhost", "port": 7000}, {"host": "localhost", "port": 7001}, {"host": "localhost", "port": 7002}]
    ...
    async def test_cluster_slot_migration(self, cluster_manager):
        ...
        mock_cluster.get.side_effect = [redis.exceptions.MovedError("MOVED 1234 localhost:7001"), "migrated_value"]
...
class TestRedisErrorRecovery:
    ...
    async def test_network_partition_recovery(self):
        ...
        config = RedisConfig(host="localhost", port=6379, db=15, retry_attempts=5, retry_delay=0.1)
