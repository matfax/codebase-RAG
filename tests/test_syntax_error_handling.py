...
from models.code_chunk import ParseResult, CodeSyntaxError, ChunkType
...
'''
        ...
        if result.error_recovery_used:
            ...
            chunks_with_parents = [chunk for chunk in result.chunks if hasattr(chunk, 'parent_name') and chunk.parent_name]
...
'''
        ...
        if len(result.syntax_errors) > 0:
            ...
            expected_types = {
                "missing_colon", "missing_punctuation", "syntax_error",
                "unexpected_token", "invalid_syntax", "parse_error"
            }
...
'''
        ...
        function_chunks = [chunk for chunk in result.chunks 
                          if chunk.chunk_type == ChunkType.FUNCTION or 'function' in str(chunk.chunk_type)]
