...
from src.tools.cache.cache_management import (
    clear_cache_tool,
    get_cache_stats_tool,
    set_cache_tool,
    warm_cache_tool
)
from src.tools.cache.cascade_invalidation_tools import (
    cascade_invalidate_tool,
    register_dependency_tool,
    get_dependency_graph_tool
)
from src.tools.core.cache_alert_management import (
    get_cache_alerts_tool,
    set_alert_threshold_tool,
...
class TestCacheManagementTools(TestMCPToolsBase):
    ...
    async def test_set_cache_tool_with_encryption(self, mock_cache_service):
        ...
        with patch('src.tools.cache.cache_management.get_cache_service', return_value=mock_cache_service):
            result = await set_cache_tool({
                "key": "secure:user:456",
                "value": {"ssn": "123-45-6789", "api_key": "secret"},
                "ttl": 1800,
                "encrypted": True
            })
...
class TestCascadeInvalidationTools(TestMCPToolsBase):
    ...
    async def test_register_dependency_tool(self, mock_invalidation_service):
        ...
        with patch('src.tools.cache.cascade_invalidation_tools.get_cascade_service', return_value=mock_cascade_service):
            result = await register_dependency_tool({
                "parent": "user:123",
                "dependents": ["profile:123", "settings:123", "preferences:123"]
            })
            
            assert result["success"] is True
            assert result["parent"] == "user:123"
            assert len(result["dependents"]) == 3
            mock_cascade_service.register_dependency.assert_called_once_with(
                "user:123",
                ["profile:123", "settings:123", "preferences:123"]
            )
    ...
    async def test_cascade_invalidate_tool(self, mock_invalidation_service):
        ...
        mock_cascade_service.invalidate_cascade.return_value = {
            ...
            "cascade_depth": 3,
            "invalidation_tree": {
                "user:123": ["profile:123", "settings:123"],
                "profile:123": ["avatar:123", "bio:123"]
            }
        }
        
        with patch('src.tools.cache.cascade_invalidation_tools.get_cascade_service', return_value=mock_cascade_service):
            result = await cascade_invalidate_tool({
                "key": "user:123",
                "max_depth": 5,
    ...
    async def test_get_dependency_graph_tool(self, mock_invalidation_service):
        ...
        mock_cascade_service.get_dependency_graph.return_value = {
            ...
            "edges": [
                {"from": "user:123", "to": "profile:123"},
                {"from": "user:123", "to": "settings:123"}
            ],
            "depth_map": {
                "user:123": 0,
                "profile:123": 1,
        ...
        with patch('src.tools.cache.cascade_invalidation_tools.get_cascade_service', return_value=mock_cascade_service):
            result = await get_dependency_graph_tool({
                "root_key": "user:123",
                "max_depth": 3
            })
...
class TestCacheAlertTools(TestMCPToolsBase):
    ...
    async def test_get_cache_alerts_tool(self):
        ...
        with patch('src.tools.core.cache_alert_management.get_alert_service', return_value=mock_alert_service):
            result = await get_cache_alerts_tool({
                "severity": "warning",
    ...
    async def test_set_alert_threshold_tool(self):
        ...
        with patch('src.tools.core.cache_alert_management.get_alert_service', return_value=mock_alert_service):
            result = await set_alert_threshold_tool({
                "metric": "memory_usage",
                "threshold": 0.9,
                "severity": "critical",
                "enabled": True
    ...
    async def test_acknowledge_alert_tool(self):
        ...
        with patch('src.tools.core.cache_alert_management.get_alert_service', return_value=mock_alert_service):
            result = await acknowledge_alert_tool({
                "alert_id": "alert_001",
                "user_id": "user123",
                "note": "Investigating memory pressure issue"
            })
...
class TestMCPToolWorkflows(TestMCPToolsBase):
    ...
    async def test_cache_management_workflow(self, mock_cache_service):
        ...
        with patch('src.tools.cache.cache_management.get_cache_service', return_value=mock_cache_service):
            ...
            set_result = await set_cache_tool({
                "key": "workflow:test:1",
                "value": {"step": 1, "data": "test"},
                "ttl": 3600
            })
    ...
    async def test_invalidation_workflow(self):
        ...
        with patch('src.tools.cache.cascade_invalidation_tools.get_cascade_service', return_value=mock_cascade_service):
            ...
            reg_result = await register_dependency_tool({
                "parent": "workflow:parent",
                "dependents": ["workflow:child1", "workflow:child2"]
            })
            assert reg_result["success"] is True
            
            # Step 2: Get dependency graph
            graph_result = await get_dependency_graph_tool({
                "root_key": "workflow:parent"
            })
            assert "nodes" in graph_result
            
            # Step 3: Cascade invalidate
            invalidate_result = await cascade_invalidate_tool({
                "key": "workflow:parent",
                "continue_on_error": True
            })
    ...
    async def test_monitoring_workflow(self):
        ...
        with patch('src.tools.core.cache_alert_management.get_alert_service', return_value=mock_alert_service):
            ...
            threshold_result = await set_alert_threshold_tool({
                "metric": "memory_usage",
                "threshold": 0.85,
                "severity": "warning"
            })
            ...
            if alerts_result["summary"]["total_alerts"] > 0:
                ack_result = await acknowledge_alert_tool({
                    "alert_id": alerts_result["alerts"][0]["id"],
                    "user_id": "system",
                    "note": "Automated acknowledgment"
                })
...
class TestMCPToolsPerformance(TestMCPToolsBase):
    ...
    async def test_batch_operations_performance(self, mock_cache_service):
        ...
        large_entries = [
            {"key": f"perf:test:{i}", "loader": "load_data", "params": {"id": i}}
            for i in range(100)
        ]
