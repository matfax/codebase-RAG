...
from src.services.file_monitoring_service import (
    FileMonitoringService,
    FileChangeEvent,
    FileChangeType
)
from src.services.file_system_event_handler import (
...
class TestCacheInvalidationService:
    ...
    async def test_conditional_invalidation(self, invalidation_service):
        ...
        result = await invalidation_service.invalidate_conditional(
            pattern="cache:*",
            condition=condition
        )
...
class TestCascadeInvalidationService:
    ...
    async def test_dependency_registration(self, cascade_service):
        ...
        await cascade_service.register_dependency(
            parent="cache:user:123",
            dependents=["cache:profile:123", "cache:settings:123"]
        )
    ...
    async def test_cascade_invalidation(self, cascade_service):
        ...
        await cascade_service.register_dependency(
            parent="cache:user:123",
            dependents=["cache:profile:123", "cache:preferences:123"]
        )
        await cascade_service.register_dependency(
            parent="cache:profile:123",
            dependents=["cache:avatar:123", "cache:bio:123"]
        )
    ...
    async def test_selective_cascade(self, cascade_service):
        ...
        result = await cascade_service.invalidate_cascade(
            "cache:product:123",
            filter_fn=lambda key: "price" in key or "stock" in key
        )
    ...
    async def test_cascade_with_failure_handling(self, cascade_service):
        ...
        await cascade_service.register_dependency(
            parent="cache:parent",
            dependents=["cache:child1", "cache:fail:child2", "cache:child3"]
        )
...
class TestFileMonitoringIntegration:
    ...
    async def test_file_change_invalidation(self, file_monitor):
        ...
        await file_monitor.register_file_cache_mapping(
            file_path="/src/models/user.py",
            cache_patterns=["cache:user:*", "cache:profile:*"]
        )
    ...
    async def test_batch_file_changes(self, file_monitor):
        ...
        # Register mappings
        mappings = {
            "/src/file1.py": ["cache:type1:*"],
            "/src/file2.py": ["cache:type2:*"],
            "/src/file3.py": ["cache:type3:*"]
        }
        ...
        events = [
            FileChangeEvent(path, FileChangeType.MODIFIED, time.time())
...
class TestInvalidationStrategies:
    ...
    async def test_ttl_based_invalidation(self):
        ...
        await service.enable_ttl_monitoring(
    ...
    async def test_memory_pressure_invalidation(self):
        ...
        mock_cache.info.return_value = {
            "used_memory": 900_000_000,  # 900MB
            "maxmemory": 1_000_000_000   # 1GB limit
        }
