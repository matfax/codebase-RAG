...
class TestConfigurationValidation:
    ...
    def test_validate_dependencies(self):
        ...
        config = SecurityConfig(
            encryption_enabled=True,
            key_rotation_interval=0  # Invalid when encryption enabled
        )
...
class TestDynamicConfiguration:
    ...
    def test_config_sources_priority(self, dynamic_config):
        ...
        # Add config sources
        dynamic_config.add_source(
            ConfigSource(name="file", priority=1, loader=lambda: {"default_ttl": 3600})
        )
        dynamic_config.add_source(
            ConfigSource(name="env", priority=2, loader=lambda: {"default_ttl": 7200})
        )
        dynamic_config.add_source(
            ConfigSource(name="api", priority=3, loader=lambda: {"default_ttl": 1800})
        )
