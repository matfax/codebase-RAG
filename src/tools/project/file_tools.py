...
from .project_utils import delete_file_chunks
        ...
        try:
            ...
            for collection_name in collections:
                ...
                try:
                    ...
                    if chunk_count > 0:
                        file_info["collections"][collection_name] = {
...
def clear_file_metadata(file_path: str, collection_name: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    """
    with log_tool_usage("clear_file_metadata", {"file_path": file_path, "collection_name": collection_name}):
        try:
...
def reindex_file(file_path: str) -> Dict[str, Any]:
    ...
    with log_tool_usage("reindex_file", {"file_path": file_path}):
        try:
            ...
            return {
                "success": True,
                "message": f"File reindexed successfully",
                "file_path": str(abs_path),
                "cleared_points": clear_result.get("deleted_points", 0),
...
def clear_file_metadata_sync(file_path: str, collection_name: Optional[str] = None) -> Dict[str, Any]:
...
def register_file_tools(mcp_app: FastMCP):
    ...
    @mcp_app.tool()
    def clear_file_metadata_tool(file_path: str, collection_name: Optional[str] = None) -> Dict[str, Any]:
        """
        Clear all chunks and metadata for a specific file.
