...
async def graph_analyze_structure(
    ...
    try:
        ...
        if not breadcrumb or not breadcrumb.strip():
            return {
                "success": False,
                "error": "Breadcrumb is required and cannot be empty",
        ...
        structure_graph = await graph_rag_service.build_structure_graph(
        ...
        if analysis_type in ["comprehensive", "connectivity"] and include_connectivity:
            connectivity_result = await graph_rag_service.analyze_component_connectivity(
                breadcrumb=breadcrumb,
                project_name=project_name
            )
        ...
        if (analysis_type == "comprehensive" and 
            results.get("related_components") and 
            len(results["related_components"]) > 1):
            try:
                # Find paths to top related components
                target_breadcrumbs = [
                    comp["breadcrumb"] for comp in results["related_components"][:3] 
                    if comp["breadcrumb"] != breadcrumb
                ]
                
                navigation_paths = []
                for target in target_breadcrumbs:
                    path = await graph_rag_service.find_hierarchical_path(
                        from_breadcrumb=breadcrumb,
                        to_breadcrumb=target,
                        project_name=project_name
                    )
                    if path:
