    ...
    try:
        ...
        if pattern_types:
            ...
            if invalid_types:
                return {
                    "success": False,
                    "error": f"Invalid pattern types: {invalid_types}. Valid types: {list(valid_pattern_types)}"
                }
        ...
        if pattern_types:
            filtered_patterns = [
                pattern for pattern in pattern_analysis.patterns_found
                if pattern.pattern_type.lower() in pattern_types
            ]
        ...
        if include_improvements and identified_patterns:
            try:
                improvement_suggestions = await pattern_service.suggest_pattern_improvements(
                    patterns=pattern_analysis.patterns_found[:5],  # Top 5 patterns
                    project_name=project_name
                )
