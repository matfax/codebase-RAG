    ...
    with log_tool_usage("diagnose_parser_health", {"comprehensive": comprehensive, "language": language}):
        try:
            ...
            if language:
                ...
                logger.info(f"Language test completed: {language} - "
...
def diagnose_parser_health_sync(comprehensive: bool = False, language: Optional[str] = None) -> Dict[str, Any]:
...
def register_parser_tools(mcp_app: FastMCP):
    ...
    @mcp_app.tool()
    def diagnose_parser_health_tool(comprehensive: bool = False, language: Optional[str] = None) -> Dict[str, Any]:
        """
        Diagnose Tree-sitter parser health and functionality.
