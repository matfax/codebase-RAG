...
def ensure_collection(collection_name: str, embedding_dimension: Optional[int] = None, 
                     embedding_model_name: Optional[str] = None, content_type: str = "general"):
            ...
            try:
                ...
                for chunk in collection_chunks:
                    if chunk.content and chunk.content.strip():
                        ...
                        logger.warning(f"Skipping empty chunk from {chunk.metadata.get('file_path', 'unknown')} "
            ...
            for i in range(0, len(points), qdrant_batch_size):
                ...
                try:
                    ...
                    if len(batch_points) > 1:
                        ...
                        individual_successes, individual_failures = retry_individual_points(
...
def index_directory_sync(
    ...
    try:
        ...
        logger.info(f"Starting streaming indexing with chunk batch size: {chunk_batch_size}, "
                   f"Qdrant batch size: {qdrant_batch_size}")
        ...
        if indexed_files_count <= MAX_FILES_IN_RESPONSE:
            ...
            logger.info(f"Truncating file list in response: showing {MAX_FILES_IN_RESPONSE} "
                       f"of {indexed_files_count} files")
