...
class FileMetadata:
    ...
    relative_path: Optional[str] = None      # Path relative to project root
    language: Optional[str] = None           # Detected programming language
    chunk_count: int = 1                     # Number of chunks created from this file
    collection_name: Optional[str] = None    # Qdrant collection where this file is stored
    
    @classmethod
    def from_file_path(cls, file_path: str, project_root: Optional[str] = None) -> 'FileMetadata':
        """
    ...
    def has_changed(self, current_mtime: float, current_size: int) -> bool:
        ...
        """
        return (
            abs(self.mtime - current_mtime) > 0.001 or  # Account for floating point precision
            self.file_size != current_size
        )
    
    def verify_content_hash(self, file_path: Optional[str] = None) -> bool:
        """
