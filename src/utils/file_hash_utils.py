...
def batch_calculate_hashes(file_paths: list[str], algorithm: str = "sha256", max_workers: int | None = None) -> dict[str, str | None]:
    """
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_path = {executor.submit(calculate_file_hash, path, algorithm): path for path in file_paths}
    ...
    logger.info(f"Batch hashed {success_count}/{len(file_paths)} files in {duration:.2f}s " f"({len(file_paths) / duration:.1f} files/sec)")
...
def find_duplicate_files(file_paths: list[str], algorithm: str = "sha256") -> dict[str, list[str]]:
    ...
    duplicates = {file_hash: files for file_hash, files in hash_to_files.items() if len(files) > 1}
...
def get_file_hash_info(file_path: str) -> dict[str, any]:
    ...
    try:
        ...
        return {"file_path": file_path, "exists": False, "error": str(e)}
