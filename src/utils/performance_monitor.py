...
class MemoryMonitor:
    ...
    def check_memory_usage(self, logger) -> Dict[str, Any]:
        ...
        try:
            ...
            if (memory_mb > self.warning_threshold_mb and 
                current_time - self._last_warning_time > self._warning_cooldown):
                
                logger.warning(
                    f"Memory usage above threshold: {memory_mb:.1f}MB "
                    f"(threshold: {self.warning_threshold_mb}MB, {memory_percent:.1f}%)"
                )
