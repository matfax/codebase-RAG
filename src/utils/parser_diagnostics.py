...
from utils.language_registry import language_registry
...
'''
    ...
    def run_comprehensive_diagnostics(self) -> ParserHealthReport:
        ...
        self.logger.info(f"Diagnostics completed in {report.test_duration_seconds:.2f}s - "
    ...
    def _test_language_parser(self, language: str) -> ParseTestResult:
        ...
        try:
            ...
            test_result.success = (test_result.node_count > 0 and 
                                 test_result.error_count <= test_result.node_count * 0.1)  # < 10% error rate
    ...
    def _generate_recommendations(self, report: ParserHealthReport) -> None:
        ...
        if report.average_parse_time_ms > 1000:
            recommendations.append(
                "Consider optimizing parsing performance by reducing file sizes or "
                "using incremental parsing for large files"
            )
        ...
        if failing_tests:
            recommendations.append(
                f"Investigate parsing issues for: {', '.join(failing_tests)}. "
                f"Check sample code compatibility and parser versions"
            )
        
        # General recommendations
        if report.overall_health_score() < 75:
            recommendations.append(
                "Run 'poetry install' to ensure all Tree-sitter dependencies are properly installed"
            )
        
        if not report.tree_sitter_available:
            recommendations.append(
                "Install Tree-sitter: pip install tree-sitter or poetry add tree-sitter"
            )
    ...
    def test_specific_language(self, language: str, sample_code: Optional[str] = None) -> ParseTestResult:
        """
    ...
    def generate_diagnostic_report(self, report: ParserHealthReport) -> str:
        ...
        if report.parsing_tests:
            ...
            for language, test in report.parsing_tests.items():
                ...
                lines.append(f"{status} {language}: {test.parse_time_ms:.1f}ms, "
                           f"{test.node_count} nodes, {test.error_count} errors")
