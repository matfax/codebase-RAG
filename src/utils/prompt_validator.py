...
class PromptValidator:
    ...
    DANGEROUS_PATTERNS = [
        ...
        r"rm\s+",
        r"del\s+",  # Deletion commands
        r"sudo\s+",
        r"su\s+",  # Privilege escalation
    ...
    def validate_directory_path(self, path: str, must_exist: bool = True) -> ValidationResult:
        ...
        if self._contains_dangerous_patterns(path):
            result.add_issue(
                ...
                "Directory path contains potentially dangerous patterns",
            )
            return result
    ...
    def validate_component_name(self, name: str) -> ValidationResult:
        ...
        if len(name) > 200:
            result.add_issue(
                ValidationSeverity.WARNING,
                "Component name is very long, may not match effectively",
            )

        if len(name) < 2:
            result.add_issue(
                ValidationSeverity.WARNING,
                "Component name is very short, may match too broadly",
            )
        ...
        if re.match(r"^[a-zA-Z_][a-zA-Z0-9_]*$", name_clean):
            result.add_issue(
                ...
                "Component name appears to be a valid identifier",
            )
        elif re.match(r"^[a-zA-Z0-9_\-\.\/]+$", name_clean):
            result.add_issue(
                ValidationSeverity.INFO,
                "Component name appears to be a file/module path",
            )
        else:
    ...
    def validate_component_type(self, component_type: str) -> ValidationResult:
        ...
        if component_type_clean not in valid_types:
            result.add_issue(
                ValidationSeverity.WARNING,
                f"Unknown component type '{component_type_clean}', using 'auto'. " f"Valid types: {', '.join(valid_types)}",
            )
            result.value = "auto"
    ...
    def validate_detail_level(self, detail_level: str) -> ValidationResult:
        ...
        if detail_level_clean not in valid_levels:
            result.add_issue(
                ValidationSeverity.WARNING,
                f"Unknown detail level '{detail_level_clean}', using 'overview'. " f"Valid levels: {', '.join(valid_levels)}",
            )
            result.value = "overview"
    ...
    def validate_user_role(self, user_role: str) -> ValidationResult:
        ...
        if user_role_clean in valid_roles:
            ...
            result.add_issue(
                ValidationSeverity.WARNING,
                f"Unknown user role '{user_role_clean}', using 'developer'. " f"Valid roles: {', '.join(valid_roles)}",
            )
            result.value = "developer"
    ...
    def validate_task_type(self, task_type: str) -> ValidationResult:
        ...
        if task_type_clean in valid_types:
            ...
            result.add_issue(
                ValidationSeverity.WARNING,
                f"Unknown task type '{task_type_clean}', using 'exploration'. " f"Valid types: {', '.join(valid_types)}",
            )
            result.value = "exploration"

        return result

    def validate_search_queries(self, queries: str | list[str]) -> ValidationResult:
        """
        ...
        if isinstance(queries, str):
            ...
            result.add_issue(
                ValidationSeverity.ERROR,
                "Search queries must be string or list of strings",
            )
            return result
        ...
        for i, query in enumerate(query_list):
            if not isinstance(query, str):
                result.add_issue(
                    ValidationSeverity.WARNING,
                    f"Query {i + 1} is not a string, skipping",
                )
                continue
            ...
            if self._contains_dangerous_patterns(query_clean):
                result.add_issue(
                    ValidationSeverity.WARNING,
                    f"Query {i + 1} contains potentially dangerous patterns, skipping",
                )
                continue
            ...
            if len(query_clean) > 500:
                result.add_issue(
                    ValidationSeverity.WARNING,
                    f"Query {i + 1} is very long, may not search effectively",
                )
    ...
    def validate_prompt_parameters(self, prompt_name: str, **kwargs) -> dict[str, ValidationResult]:
        ...
        self.logger.info(
            f"Validation complete for {prompt_name}: " f"{valid_params}/{total_params} valid parameters, " f"{issues_count} issues found"
        )
