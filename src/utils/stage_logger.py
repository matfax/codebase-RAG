...
class StageLogger:
    ...
    def __init__(self, name: str, parent_logger: Optional[logging.Logger] = None):
        self.name = name
    ...
    def get_all_stage_summaries(self) -> Dict[str, Any]:
        ...
        return {
            name: self.get_stage_summary(name) 
            for name in self._stages.keys()
        }
...
def create_stage_logger(name: str, parent_logger: Optional[logging.Logger] = None) -> StageLogger:
...
def log_batch_summary(logger: StageLogger, batch_num: int, batch_size: int, 
                     processed: int, failed: int, duration: float) -> None:
