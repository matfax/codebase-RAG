...
def register_mcp_tools(mcp_app: FastMCP):
    ...
    def search(
        ...
        try:
            ...
            query_embedding_tensor = embeddings_manager.generate_embeddings(
                os.getenv("OLLAMA_DEFAULT_EMBEDDING_MODEL", "nomic-embed-text"), query
            )

            # Handle embedding generation failure
            if query_embedding_tensor is None:
                return {
                    "error": "Failed to generate embedding for query (empty query or embedding error)",
                    "query": query,
                }
            ...
            if include_context and all_results:
                all_results = _expand_search_context(
                    all_results, qdrant_client, search_collections, context_chunks, 768
                )  # Updated dimension
...
def analyze_repository(directory: str = ".") -> dict[str, Any]:
    ...
    try:
        ...
        # Log key statistics
        console_logger.info("Repository analysis complete:")
...
def get_file_filtering_stats(directory: str = ".") -> dict[str, Any]:
    ...
    try:
        ...
        console_logger.info("File filtering analysis complete:")
