...
class ExtractionContext:
    ...
    statistics: dict[str, int] = field(
        default_factory=lambda: {"nodes_processed": 0, "edges_processed": 0, "paths_extracted": 0, "paths_filtered": 0}
    )
...
class PathBasedIndexer:
    ...
    def __init__(self, lightweight_graph_service: LightweightGraphService, graph_rag_service: GraphRAGService | None = None):
        """
    ...
    async def extract_relational_paths(
        self, project_name: str, entry_points: list[str] | None = None, options: ExtractionOptions | None = None
    ) -> PathExtractionResult:
        """
        ...
        try:
            ...
            target_nodes = await self._determine_extraction_scope(structure_graph, entry_points, options)
            ...
            if options.enable_execution_paths:
                extraction_tasks.append(self._extract_execution_paths(structure_graph, target_nodes, options, context))

            if options.enable_data_flow_paths:
                extraction_tasks.append(self._extract_data_flow_paths(structure_graph, target_nodes, options, context))

            if options.enable_dependency_paths:
                extraction_tasks.append(self._extract_dependency_paths(structure_graph, target_nodes, options, context))
            ...
            result = PathExtractionResult(
                ...
                paths_with_high_confidence=sum(
                    1
                    for path in collection.execution_paths + collection.data_flow_paths + collection.dependency_paths
            ...
            self.logger.info(
                f"Path extraction completed: {result.path_collection.get_total_path_count()} paths " f"in {processing_time_ms:.2f}ms"
            )
    ...
    async def _get_project_structure_graph(self, project_name: str) -> StructureGraph | None:
    ...
    async def _determine_extraction_scope(
        self, graph: StructureGraph, entry_points: list[str] | None, options: ExtractionOptions
    ) -> list[GraphNode]:
    ...
    async def _extract_execution_paths(
        self, graph: StructureGraph, target_nodes: list[GraphNode], options: ExtractionOptions, context: ExtractionContext
    ) -> list[ExecutionPath]:
    ...
    def _is_high_confidence_path(self, path: AnyPath) -> bool:
        ...
        high_confidence_edges = sum(1 for edge in path.edges if edge.confidence in {PathConfidence.HIGH, PathConfidence.VERY_HIGH})
    ...
    def _create_empty_result(self, project_name: str, error_message: str) -> PathExtractionResult:
        ...
        collection = RelationalPathCollection(collection_id=f"{project_name}_empty", collection_name=f"Empty Collection for {project_name}")

        return PathExtractionResult(
            path_collection=collection, processing_time_ms=0.0, source_chunks_count=0, success_rate=0.0, extraction_warnings=[error_message]
        )
    ...
    def _update_performance_stats(self, processing_time_ms: float, paths_extracted: int, success: bool):
        ...
        self._extraction_stats["average_extraction_time_ms"] = (
            current_avg * (total_extractions - 1) + processing_time_ms
        ) / total_extractions
...
class ExecutionPathExtractor:
    ...
    async def extract_from_node(
        self, start_node: GraphNode, graph: StructureGraph, options: ExtractionOptions, context: ExtractionContext
    ) -> list[ExecutionPath]:
        """Extract execution paths starting from a given node."""
        paths = []

        # Find function call edges from this node
        call_edges = [edge for edge in graph.edges if edge.source_breadcrumb == start_node.breadcrumb and edge.is_function_call()]
    ...
    async def _build_execution_path(
        self, start_node: GraphNode, initial_edge: GraphEdge, graph: StructureGraph, options: ExtractionOptions, context: ExtractionContext
    ) -> ExecutionPath | None:
        ...
        try:
            ...
            while current_edge and depth < options.max_path_length:
                ...
                next_edges = [
                    ...
                    for edge in graph.edges
                    if (
                        edge.source_breadcrumb == target_node.breadcrumb
                        and edge.is_function_call()
...
class DependencyPathExtractor:
    ...
    async def extract_from_node(
        ...
        try:
            ...
            dep_edges = [edge for edge in graph.edges if edge.source_breadcrumb == start_node.breadcrumb and edge.is_dependency()]
...
def create_path_based_indexer(
    lightweight_graph_service: LightweightGraphService, graph_rag_service: GraphRAGService | None = None
) -> PathBasedIndexer:
    """
