...
class RAGSearchStrategy:
    ...
    def execute_focused_search(self, search_query: SearchQuery, project_path: str | None = None) -> list[SearchResult]:
        """
    ...
    def _build_search_queries(self, focus_areas: list[SearchQueryType] | None, max_results: int) -> list[SearchQuery]:
    ...
    def _calculate_metadata_bonus(self, result: dict[str, Any], query_type: SearchQueryType) -> float:
        ...
        if query_type == SearchQueryType.ENTRY_POINTS and chunk_type == "function":
            bonus += 0.3
        elif query_type == SearchQueryType.CORE_COMPONENTS and chunk_type in [
            "class",
            "function",
        ]:
            bonus += 0.2
    ...
    def detect_architecture_patterns(self, project_path: str | None = None, max_results: int = 10) -> dict[str, Any]:
        """
    ...
    def discover_entry_points(self, project_path: str | None = None, max_results: int = 10) -> dict[str, Any]:
        """
    ...
    def _extract_entry_point_metadata(self, search_result: SearchResult) -> dict[str, Any]:
        ...
        search_result.original_result.get("signature", "")
        search_result.original_result.get("file_path", "")
    ...
    def analyze_component_relationships(
        ...
        try:
            ...
            relationships = self._analyze_pairwise_relationships(core_components, similarity_threshold, project_path)
            ...
            self.logger.info("Component relationship analysis completed")
    ...
    def _discover_core_components(self, project_path: str | None, max_components: int) -> list[dict[str, Any]]:
        """
    ...
    def _determine_relationship_type(self, comp1: dict[str, Any], comp2: dict[str, Any], similarity: float) -> str:
        """
    ...
    def _calculate_relationship_strength(self, comp1: dict[str, Any], comp2: dict[str, Any], similarity: float) -> str:
        """
    ...
    def _identify_component_clusters(self, components: list[dict[str, Any]], relationships: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """
    ...
    def _dfs_cluster(
        ...
        while stack:
            ...
            if current_key in comp_lookup:
                ...
                for neighbor_key, _similarity in adjacency.get(current_key, []):
    ...
    def _analyze_dependency_patterns(self, relationships: list[dict[str, Any]], components: list[dict[str, Any]]) -> dict[str, Any]:
        """
        ...
        hub_components = sorted(component_connections.items(), key=lambda x: x[1], reverse=True)[:5]
    ...
    def _extract_architectural_insights(self, clusters: list[dict[str, Any]], relationships: list[dict[str, Any]]) -> list[str]:
        """
        ...
        if clusters:
            ...
            insights.append(f"Largest component cluster contains {largest_cluster['cluster_size']} related components")

            if len(clusters) > 3:
                insights.append(f"Identified {len(clusters)} distinct component clusters, suggesting modular architecture")
        ...
        if relationships:
            ...
            if strong_relationships:
                insights.append(f"Found {len(strong_relationships)} strong component relationships indicating tight coupling")
            ...
            controller_service_rels = [
                r for r in relationships if "controller" in r["relationship_type"] and "service" in r["relationship_type"]
            ]
