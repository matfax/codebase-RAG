...
from ..services.cache_service import get_cache_service, BaseCacheService
...
class CacheFailoverService:
    ...
    def __init__(self, config: Optional[CacheConfig] = None, failover_config: Optional[FailoverConfiguration] = None):
        ...
        # Service registry
        self._primary_service: Optional[BaseCacheService] = None
        self._failover_services: List[BaseCacheService] = []
        self._current_service: Optional[BaseCacheService] = None
        ...
        self._auto_recovery_task: Optional[asyncio.Task] = None
        ...
        for attempt in range(max_attempts):
            try:
                ...
                result = await asyncio.wait_for(
                    method(*args, **kwargs),
                    timeout=self.failover_config.health_check_timeout_seconds
                )
    ...
    async def _check_service_health(self, service: BaseCacheService):
        ...
        try:
            ...
            await asyncio.wait_for(
                service.set(test_key, "health_check"),
                timeout=self.failover_config.health_check_timeout_seconds
            )
    ...
    async def _trigger_failover(self, trigger: FailoverTrigger, details: Dict[str, Any]) -> FailoverEvent:
        ...
        try:
            ...
            self.logger.warning(
                f"Failover triggered: {trigger.value}, "
                f"switched from {current_service_id} to {failover_service_id}"
            )
    ...
    async def _select_best_failover_service(self) -> Optional[BaseCacheService]:
    ...
    async def _attempt_recovery(self) -> bool:
        ...
        try:
            ...
            if consecutive_successes >= self.failover_config.recovery_threshold:
                ...
                self._failover_status = FailoverStatus.FAILED_OVER
                self.logger.info(f"Primary service not yet ready for recovery, need {self.failover_config.recovery_threshold - consecutive_successes} more successful checks")
                return False
    ...
    def _get_service_by_id(self, service_id: str) -> Optional[BaseCacheService]:
        """Get service by ID."""
