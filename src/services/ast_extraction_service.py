...
from models.code_chunk import CodeChunk, ChunkType, CodeSyntaxError
...
class AstExtractionService:
    ...
    def extract_chunks(self, root_node: Node, file_path: str, content: str, 
                      language: str, node_mappings: Dict[ChunkType, List[str]]) -> List[CodeChunk]:
        """
        ...
        if error_count == 0:
            ...
            self._traverse_ast(root_node, chunks, file_path, content, 
                             content_lines, language, node_mappings)
        else:
            ...
            self._traverse_ast_with_error_filtering(
                root_node, chunks, file_path, content, content_lines, 
                language, node_mappings, error_lines
            )
        
        return chunks
    
    def traverse_ast(self, node: Node, chunks: List[CodeChunk], file_path: str, 
    ...
    def _traverse_ast(self, node: Node, chunks: List[CodeChunk], file_path: str,
        ...
        for child in node.children:
            self._traverse_ast(child, chunks, file_path, content, 
                             content_lines, language, node_mappings)
    ...
    def _traverse_ast_with_error_filtering(self, node: Node, chunks: List[CodeChunk],
        ...
        for child in node.children:
            self._traverse_ast_with_error_filtering(
                child, chunks, file_path, content, content_lines, 
                language, node_mappings, error_lines
            )
    
    def traverse_for_errors(self, node: Node, content_lines: List[str], 
    ...
    def get_chunk_type(self, node: Node, node_mappings: Dict[ChunkType, List[str]], 
                      language: str) -> Optional[ChunkType]:
        """
            ...
            if node_type in node_types:
                ...
                if chunk_type == ChunkType.CONSTANT:
                    ...
                    if language == 'python' and not self.is_module_level_assignment(node):
                        continue
                    elif language in ['javascript', 'typescript'] and not self.is_module_level_js_declaration(node):
                        continue
    ...
    def extract_name(self, node: Node, language: str) -> Optional[str]:
        """
    ...
    def extract_signature(self, node: Node, language: str) -> Optional[str]:
        """
    ...
    def extract_docstring(self, node: Node, content_lines: List[str], language: str) -> Optional[str]:
        """
    ...
    def _extract_js_signature(self, node: Node) -> Optional[str]:
        ...
        if node.type in ['function_declaration', 'async_function_declaration', 'method_definition']:
    ...
    def create_chunk_from_node(self, node: Node, chunk_type: ChunkType, file_path: str,
    ...
    def extract_context_before(self, lines: List[str], start_idx: int, num_lines: int) -> Optional[str]:
    ...
    def extract_context_after(self, lines: List[str], end_idx: int, num_lines: int) -> Optional[str]:
