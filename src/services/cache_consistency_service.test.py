...
class TestCacheConsistencyService:
    ...
    async def mock_cache_service(self):
        ...
        cache_service._l1_cache = {
            "test_key_1": CacheEntry(data={"test": "data1"}, checksum="abc123", size=20, created_at=datetime.now()),
            "test_key_2": CacheEntry(data={"test": "data2"}, checksum="def456", size=20, created_at=datetime.now()),
        }
    ...
    async def test_basic_consistency_check(self, consistency_service, mock_cache_service):
        ...
        with patch.object(consistency_service, "_get_cache_service", return_value=mock_cache_service):
            report = await consistency_service.verify_consistency(check_level=ConsistencyCheckLevel.BASIC)
    ...
    async def test_l1_l2_mismatch_detection(self, consistency_service, mock_cache_service):
        ...
        with patch.object(consistency_service, "_get_cache_service", return_value=mock_cache_service):
            report = await consistency_service.verify_consistency(check_level=ConsistencyCheckLevel.COMPREHENSIVE)

            # Should detect mismatch for test_key_2
            l1_l2_issues = [issue for issue in report.issues_found if issue.issue_type == ConsistencyIssueType.L1_L2_MISMATCH]
    ...
    async def test_orphaned_entry_detection(self, consistency_service, mock_cache_service):
        ...
        with patch.object(consistency_service, "_get_cache_service", return_value=mock_cache_service):
            report = await consistency_service.verify_consistency(check_level=ConsistencyCheckLevel.DEEP)
            ...
            orphan_issues = [issue for issue in report.issues_found if issue.issue_type == ConsistencyIssueType.ORPHANED_ENTRY]
    ...
    async def test_data_integrity_validation(self, consistency_service, mock_cache_service):
        ...
        corrupted_entry = CacheEntry(data={"test": "data"}, checksum="invalid_checksum", size=15, created_at=datetime.now())
        ...
        with patch.object(consistency_service, "_get_cache_service", return_value=mock_cache_service):
            ...
            checksum_issues = [issue for issue in issues if issue.issue_type == ConsistencyIssueType.INVALID_CHECKSUM]
    ...
    async def test_expiration_validation(self, consistency_service, mock_cache_service):
        ...
        with patch.object(consistency_service, "_get_cache_service", return_value=mock_cache_service):
            ...
            expired_issues = [issue for issue in issues if issue.issue_type == ConsistencyIssueType.EXPIRED_DATA]
    ...
    async def test_metadata_validation(self, consistency_service, mock_cache_service):
        ...
        invalid_size_entry = CacheEntry(
            ...
            checksum="abc123",
            size=5,
            created_at=datetime.now(),  # Incorrect size
        )
        ...
        with patch.object(consistency_service, "_get_cache_service", return_value=mock_cache_service):
            ...
            metadata_issues = [issue for issue in issues if issue.issue_type == ConsistencyIssueType.METADATA_MISMATCH]
    ...
    async def test_consistency_score_calculation(self, consistency_service):
        ...
        issues = [
            ConsistencyIssue(issue_type=ConsistencyIssueType.L1_L2_MISMATCH, cache_key="key1", description="Test", severity="low"),
            ConsistencyIssue(issue_type=ConsistencyIssueType.CORRUPTED_DATA, cache_key="key2", description="Test", severity="critical"),
        ]
    ...
    async def test_recommendation_generation(self, consistency_service):
        ...
        issues = [
            ConsistencyIssue(issue_type=ConsistencyIssueType.L1_L2_MISMATCH, cache_key="key1", description="Test", severity="medium"),
            ConsistencyIssue(issue_type=ConsistencyIssueType.CORRUPTED_DATA, cache_key="key2", description="Test", severity="high"),
        ]
    ...
    async def test_comprehensive_consistency_check(self, consistency_service, mock_cache_service):
        ...
        with patch.object(consistency_service, "_get_cache_service", return_value=mock_cache_service):
            report = await consistency_service.verify_consistency(check_level=ConsistencyCheckLevel.COMPREHENSIVE, fix_issues=True)
    ...
    def test_values_equal_comparison(self, consistency_service):
        ...
        assert consistency_service._values_equal(
            {"list": [1, 2, 3], "dict": {"nested": True}}, {"list": [1, 2, 3], "dict": {"nested": True}}
        )
    ...
    def test_data_validation(self, consistency_service):
        ...
        valid_metadata = CacheMetadata(created_at=datetime.now() - timedelta(minutes=5), expires_at=datetime.now() + timedelta(hours=1))
        assert consistency_service._is_metadata_valid(valid_metadata)

        # Test invalid metadata (created in future)
        invalid_metadata = CacheMetadata(created_at=datetime.now() + timedelta(hours=1), expires_at=datetime.now() + timedelta(hours=2))
