...
class BackupMetadata:
    ...
    base_backup_id: str | None = None  # For incremental/differential backups
...
class CacheBackupService:
    ...
    def __init__(self, config: CacheConfig | None = None, backup_config: BackupConfiguration | None = None):
        """Initialize the cache backup service."""
        self.config = config or get_global_cache_config()
        self.backup_config = backup_config or BackupConfiguration(backup_directory=Path("./cache_backups"))
    ...
    async def create_backup(
        ...
        try:
            ...
            if backup_type == BackupType.FULL:
                ...
                cache_data = await self._collect_incremental_backup_data(cache_service, tiers, base_backup_id)
            elif backup_type == BackupType.DIFFERENTIAL:
                cache_data = await self._collect_differential_backup_data(cache_service, tiers, base_backup_id)
            ...
            original_size = await self._write_backup_data(cache_data, backup_file, compress, encrypt)
    ...
    async def restore_from_backup(
        ...
        try:
            ...
            self.logger.info(f"Restore completed: {restore_id}, " f"entries: {restore_op.restored_entries}, " f"strategy: {strategy.value}")
    ...
    async def list_backups(self, backup_type: BackupType | None = None, max_age_days: int | None = None) -> list[BackupMetadata]:
        """
    ...
    async def verify_backup_integrity(self, backup_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                return {"valid": False, "error": f"Cannot load backup data: {e}", "checksum_valid": checksum_valid}
    ...
    async def _collect_incremental_backup_data(self, cache_service, tiers: list[str], base_backup_id: str) -> dict[str, Any]:
    ...
    async def _collect_differential_backup_data(self, cache_service, tiers: list[str], base_backup_id: str) -> dict[str, Any]:
        ...
        if tier == "L1" and hasattr(cache_service, "_l1_cache") and cache_service._l1_cache:
            ...
            for key in keys:
                ...
                try:
                    ...
                    if data:
                        tier_data[key] = {"data": data, "metadata": {"ttl": await redis.ttl(key), "type": await redis.type(key)}}
                except Exception as e:
    ...
    async def _write_backup_data(self, data: dict[str, Any], backup_file: Path, compress: bool, encrypt: bool) -> int:
        ...
        while True:
            try:
                ...
                if hours_since_incremental >= self.backup_config.incremental_backup_interval_hours:
                    ...
                    if last_full:
                        await self.create_backup(BackupType.INCREMENTAL, base_backup_id=last_full.backup_id)
    ...
    async def _get_last_backup_time(self, backup_type: BackupType) -> datetime | None:
