...
class ComponentAnalysisResult:
    ...
    # Component details
    primary_component: Optional[ComponentMatch] = None
...
class ComponentAnalysisService:
    ...
    def _search_file_for_component(self, file_path: Path, component_name: str, patterns: List[Dict[str, Any]]) -> Dict[str, List[ComponentMatch]]:
                    ...
                    for line_num, line in enumerate(lines, 1):
                        if re.search(pattern, line, re.IGNORECASE):
                            match = self._create_component_match(
                                file_path, component_name, component_type, 
                                line_num, line, content, lines
                            )
    ...
    def _create_component_match(self, file_path: Path, component_name: str, component_type: str, 
                              line_num: int, line: str, content: str, lines: List[str]) -> ComponentMatch:
    ...
    def _extract_function_calls(self, content: str) -> List[str]:
        ...
        common_keywords = {'if', 'for', 'while', 'def', 'class', 'return', 'print', 'len', 'str', 'int'}
    ...
    def _classify_usage_type(self, line: str, component_name: str) -> Optional[str]:
        """Classify the type of component usage in a line."""
        line_lower = line.lower()
        name_lower = component_name.lower()
        
        if f"import {name_lower}" in line_lower or f"from " in line_lower and name_lower in line_lower:
            return "import"
        elif f"{name_lower}(" in line_lower:
            return "instantiation"
        elif f".{name_lower}(" in line_lower:
            return "method_call"
        elif f"class " in line_lower and name_lower in line_lower:
            return "inheritance"
        elif f"{name_lower}." in line_lower:
