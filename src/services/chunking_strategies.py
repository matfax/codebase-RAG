...
from models.code_chunk import CodeChunk, ChunkType
...
class ChunkingStrategyRegistry:
    ...
    def get_strategy(self, language: str) -> Optional[BaseChunkingStrategy]:
        """
            ...
            if isinstance(data, dict):
                for i, (key, value) in enumerate(data.items()):
                    ...
                    chunk = self._create_structured_chunk(
                        file_path, chunk_content, ChunkType.OBJECT,
                        name=key, start_line=i + 1, end_line=i + 1
                    )
                    chunks.append(chunk)
    ...
    def _extract_markdown_chunks(self, file_path: str, content: str) -> List[CodeChunk]:
        ...
        for i, line in enumerate(lines):
            if line.startswith('#'):
                ...
                if current_section and current_header:
                    ...
                    chunk = self._create_structured_chunk(
                        file_path, section_content, ChunkType.SECTION,
                        name=current_header.strip('#').strip(),
                        start_line=section_start, end_line=i
                    )
                    chunks.append(chunk)
        ...
        if current_section and current_header:
            ...
            chunk = self._create_structured_chunk(
                file_path, section_content, ChunkType.SECTION,
                name=current_header.strip('#').strip(),
                start_line=section_start, end_line=len(lines)
            )
            chunks.append(chunk)
        
        return chunks if chunks else [self._extract_single_chunk(file_path, content)]
    
    def _create_structured_chunk(self, file_path: str, content: str, chunk_type: ChunkType,
                                name: str, start_line: int, end_line: int) -> CodeChunk:
...
class PythonChunkingStrategy(BaseChunkingStrategy):
    ...
    def extract_chunks(self, root_node: Node, file_path: str, content: str) -> List[CodeChunk]:
        ...
        chunks = self.ast_extractor.extract_chunks(
            root_node, file_path, content, self.language, node_mappings
        )
    ...
    def _is_valid_python_chunk(self, chunk: CodeChunk) -> bool:
        ...
        if chunk.chunk_type == ChunkType.FUNCTION:
            ...
            return (chunk.name and 
                   chunk.name.isupper() and 
                   '_' in chunk.name and
                   len(chunk.content.strip()) < 200)  # Avoid huge constants
