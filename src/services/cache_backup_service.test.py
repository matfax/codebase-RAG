...
class TestCacheBackupService:
    ...
    async def mock_cache_service(self):
        ...
        test_entries = {
            "test_key_1": CacheEntry(
                data={"content": "test data 1", "type": "test"}, size=100, created_at=datetime.now(), checksum="abc123"
            ),
            "test_key_2": CacheEntry(
                data={"content": "test data 2", "type": "test"}, size=150, created_at=datetime.now(), checksum="def456"
            ),
            "test_key_3": CacheEntry(
                data={"content": "test data 3", "type": "test"}, size=200, created_at=datetime.now(), checksum="ghi789"
            ),
        }
    ...
    async def test_create_full_backup(self, backup_service, mock_cache_service):
        ...
        with patch.object(backup_service, "_get_cache_service", return_value=mock_cache_service):
            metadata = await backup_service.create_backup(backup_type=BackupType.FULL, tiers=["L1", "L2"])
    ...
    async def test_create_incremental_backup(self, backup_service, mock_cache_service):
        ...
        with patch.object(backup_service, "_get_cache_service", return_value=mock_cache_service):
            ...
            incremental_backup = await backup_service.create_backup(
                backup_type=BackupType.INCREMENTAL, base_backup_id=full_backup.backup_id
            )
    ...
    async def test_backup_compression(self, backup_service, mock_cache_service):
        ...
        with patch.object(backup_service, "_get_cache_service", return_value=mock_cache_service):
            ...
            metadata = await backup_service.create_backup(backup_type=BackupType.FULL, compress=True)
    ...
    async def test_restore_replace_all(self, backup_service, mock_cache_service):
        ...
        with patch.object(backup_service, "_get_cache_service", return_value=mock_cache_service):
            ...
            restore_op = await backup_service.restore_from_backup(
                backup_id=backup_metadata.backup_id, strategy=RecoveryStrategy.REPLACE_ALL, target_tiers=["L1", "L2"]
            )
    ...
    async def test_restore_merge_preserve(self, backup_service, mock_cache_service):
        ...
        with patch.object(backup_service, "_get_cache_service", return_value=mock_cache_service):
            ...
            restore_op = await backup_service.restore_from_backup(
                backup_id=backup_metadata.backup_id, strategy=RecoveryStrategy.MERGE_PRESERVE_EXISTING, target_tiers=["L1"]
            )
    ...
    async def test_restore_selective(self, backup_service, mock_cache_service):
        ...
        with patch.object(backup_service, "_get_cache_service", return_value=mock_cache_service):
            ...
            restore_op = await backup_service.restore_from_backup(
                backup_id=backup_metadata.backup_id, strategy=RecoveryStrategy.SELECTIVE_RESTORE, selective_keys=selective_keys
            )
    ...
    async def test_dry_run_restore(self, backup_service, mock_cache_service):
        ...
        with patch.object(backup_service, "_get_cache_service", return_value=mock_cache_service):
            ...
            restore_op = await backup_service.restore_from_backup(
                backup_id=backup_metadata.backup_id, strategy=RecoveryStrategy.REPLACE_ALL, dry_run=True
            )
    ...
    async def test_incremental_data_filtering(self, backup_service, mock_cache_service):
        ...
        with patch.object(backup_service, "_get_cache_service", return_value=mock_cache_service):
            ...
            incremental_data = await backup_service._get_tier_data_since(mock_cache_service, "L1", cutoff_time)
    ...
    async def test_backup_configuration_validation(self, temp_backup_dir):
        ...
        config = BackupConfiguration(backup_directory=temp_backup_dir, max_backup_age_days=30, max_backup_count=100)
    ...
    async def test_backup_with_encryption_disabled(self, backup_service, mock_cache_service):
        ...
        with patch.object(backup_service, "_get_cache_service", return_value=mock_cache_service):
            metadata = await backup_service.create_backup(backup_type=BackupType.FULL, encrypt=False)
    ...
    async def test_multiple_tier_backup_and_restore(self, backup_service, mock_cache_service):
        ...
        with patch.object(backup_service, "_get_cache_service", return_value=mock_cache_service):
            ...
            backup_metadata = await backup_service.create_backup(backup_type=BackupType.FULL, tiers=["L1", "L2"])
            ...
            restore_op = await backup_service.restore_from_backup(
                backup_id=backup_metadata.backup_id, target_tiers=["L1", "L2"], strategy=RecoveryStrategy.REPLACE_ALL
            )
