...
class ConsistencyIssue:
    ...
    l1_value: Any | None = None
    l2_value: Any | None = None
...
class CacheConsistencyService:
    ...
    async def verify_consistency(
        ...
        self.logger.info(
            f"Consistency check completed: {len(issues)} issues found, " f"score: {consistency_score:.3f}, duration: {duration:.2f}s"
        )
                        ...
                        if calculated_checksum != entry.checksum:
                            issues.append(
                                ConsistencyIssue(
                                    ...
                                    severity="high",
                                    metadata={"expected_checksum": entry.checksum, "actual_checksum": calculated_checksum},
                        ...
                        if entry.expires_at < current_time:
                            issues.append(
                                ConsistencyIssue(
                                    ...
                                    severity="low",
                                    metadata={"expired_at": entry.expires_at.isoformat(), "current_time": current_time.isoformat()},
                        ...
                        if abs(actual_size - entry.size) > (entry.size * 0.1):  # 10% tolerance
                            issues.append(
                                ConsistencyIssue(
                                    ...
                                    severity="low",
                                    metadata={"reported_size": entry.size, "actual_size": actual_size},
    ...
    async def _fix_consistency_issues(self, cache_service, issues: list[ConsistencyIssue]) -> dict[str, int]:
        ...
        fix_results = {"fixed": 0, "failed": 0, "skipped": 0}
