...
class TestExecutionPathExtractor:
    ...
    def setup_method(self):
        ...
        self.graph = StructureGraph(
            nodes={"module.function1": self.node1, "module.function2": self.node2},
            edges=[
                GraphEdge(
    ...
    async def test_extract_from_node_no_calls(self):
        ...
        empty_graph = StructureGraph(nodes={"module.function1": self.node1}, edges=[], project_name="test_project")
    ...
    async def test_extract_with_max_depth_limit(self):
        ...
        extended_graph = StructureGraph(
            nodes={"module.function1": self.node1, "module.function2": self.node2, "module.function3": node3},
            edges=[
                GraphEdge(
                    source_breadcrumb="module.function1",
                    target_breadcrumb="module.function2",
...
class TestDependencyPathExtractor:
    ...
    def setup_method(self):
        ...
        self.graph = StructureGraph(
            nodes={"module.imports.requests": self.import_node, "external.requests": self.module_node},
            edges=[
                GraphEdge(
    ...
    async def test_extract_no_dependencies(self):
        ...
        empty_graph = StructureGraph(nodes={"module.imports.requests": self.import_node}, edges=[], project_name="test_project")
...
class TestPathBasedIndexer:
    ...
    def setup_method(self):
        ...
        # Create indexer
        self.indexer = PathBasedIndexer(self.mock_lightweight_graph, self.mock_graph_rag_service)
    ...
    async def test_extract_relational_paths_with_entry_points(self):
        ...
        result = await self.indexer.extract_relational_paths("test_project", entry_points=["main.function1"])
    ...
    async def test_extract_relational_paths_with_options(self):
        ...
        options = ExtractionOptions(
            enable_execution_paths=True, enable_data_flow_paths=False, enable_dependency_paths=False, max_paths_per_type=10
        )

        result = await self.indexer.extract_relational_paths("test_project", options=options)
    ...
    async def test_determine_extraction_scope_with_entry_points(self):
        ...
        target_nodes = await self.indexer._determine_extraction_scope(self.sample_graph, entry_points, options)
    ...
    async def test_determine_extraction_scope_no_entry_points(self):
        ...
        target_nodes = await self.indexer._determine_extraction_scope(self.sample_graph, None, options)
    ...
    def test_filter_duplicate_execution_paths(self):
        ...
        path1 = ExecutionPath(path_id="path1", nodes=[Mock(breadcrumb="a"), Mock(breadcrumb="b")])
        path2 = ExecutionPath(path_id="path2", nodes=[Mock(breadcrumb="a"), Mock(breadcrumb="b")])
        path3 = ExecutionPath(path_id="path3", nodes=[Mock(breadcrumb="c"), Mock(breadcrumb="d")])
    ...
    def test_calculate_coverage_score(self):
        ...
        target_nodes = [Mock(breadcrumb="a"), Mock(breadcrumb="b"), Mock(breadcrumb="c")]

        paths = [Mock(nodes=[Mock(breadcrumb="a"), Mock(breadcrumb="b")]), Mock(nodes=[Mock(breadcrumb="c")])]
    ...
    def test_calculate_coherence_score(self):
        ...
        paths = [Mock(nodes=[Mock(breadcrumb="a"), Mock(breadcrumb="b")]), Mock(nodes=[Mock(breadcrumb="b"), Mock(breadcrumb="c")])]
    ...
    def test_is_high_confidence_path(self):
        ...
        high_confidence_path = Mock(edges=[Mock(confidence=PathConfidence.HIGH), Mock(confidence=PathConfidence.VERY_HIGH)])

        assert self.indexer._is_high_confidence_path(high_confidence_path)

        # Low confidence path
        low_confidence_path = Mock(edges=[Mock(confidence=PathConfidence.LOW), Mock(confidence=PathConfidence.MEDIUM)])
