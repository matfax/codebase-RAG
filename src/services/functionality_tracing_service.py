...
class FunctionalityTracingService:
    ...
    def trace_functionality(
        ...
        trace = FunctionalityTrace(
            functionality_name=functionality_description,
    ...
    def _build_call_tree(self, entry_points: List[CallNode], project_path: Path, max_depth: int) -> Tuple[Optional[CallNode], List[List[CallNode]]]:
    ...
    def _find_function_definition(self, function_name: str, project_path: Path, current_file: Path) -> Optional[CallNode]:
    ...
    def _analyze_configuration_dependencies(self, execution_paths: List[List[CallNode]], project_path: Path) -> List[ConfigurationDependency]:
