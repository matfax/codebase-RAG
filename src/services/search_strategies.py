...
class BaseSearchStrategy(ABC):
    ...
    def _enrich_search_result(self, result: Any) -> EnrichedSearchResult | None:
        """
...
class SearchStrategyRegistry:
    ...
    def get_strategy(self, name: str) -> BaseSearchStrategy | None:
        """
    ...
    def execute_parallel_searches(self, queries: list[str], search_mode: SearchMode = None) -> dict[str, list[Any]]:
        ...
        try:
            with ThreadPoolExecutor(max_workers=min(len(queries), 5)) as executor:
                # Submit all queries
                future_to_query = {executor.submit(self.execute_search, query, search_mode): query for query in queries}
...
class SemanticSearchStrategy(BaseSearchStrategy):
    ...
    def search(self, query: str, **kwargs) -> list[EnrichedSearchResult]:
        ...
        filtered_results = [result for result in enriched_results if result.score >= semantic_threshold]
...
class KeywordSearchStrategy(BaseSearchStrategy):
    ...
    def search(self, query: str, **kwargs) -> list[EnrichedSearchResult]:
        ...
        filtered_results = [result for result in enriched_results if len(result.keyword_matches) >= min_keyword_matches]

        # Sort by keyword match quality and confidence
        filtered_results.sort(key=lambda x: (len(x.keyword_matches), x.confidence_score), reverse=True)
...
class HybridSearchStrategy(BaseSearchStrategy):
    ...
    def search(self, query: str, **kwargs) -> list[EnrichedSearchResult]:
        ...
        if combine_strategies:
            ...
            semantic_results = self.semantic_strategy.search(query, semantic_threshold=kwargs.get("semantic_threshold", 0.4))

            # Get keyword results
            keyword_results = self.keyword_strategy.search(query, min_keyword_matches=kwargs.get("min_keyword_matches", 1))
        ...
        for result in all_results:
            result.confidence_score = self._calculate_hybrid_confidence(result, semantic_weight, keyword_weight)
    ...
    def _combine_results(
        self,
        base_results: list[EnrichedSearchResult],
        ...
        for result in semantic_results:
            ...
            if key in result_map:
                ...
                existing.confidence_score = max(existing.confidence_score, result.confidence_score * semantic_weight)
            else:
        ...
        for result in keyword_results:
            ...
            if key in result_map:
                # Boost existing result
                existing = result_map[key]
                existing.confidence_score = max(existing.confidence_score, result.confidence_score * keyword_weight)
            else:
